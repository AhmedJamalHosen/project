    ...
    def del_root(self,deleted_root):
		...
		for dp in self.datapaths.values():
			...
			ofproto = dp.ofproto
		        ...
		        mod = parser.OFPFlowMod(datapath=datapath,cookie=cookie,cookie_mask=cookie_mask,command=ofproto.OFPFC_DELETE,out_port=ofproto.OFPP_ANY,
    ...
    def send_aggregate_stats_request(self):
	    ...
	    for dp in self.datapaths.values():
			...
			if dp.id == 7 or dp.id ==8:
			    ...
			    req = ofp_parser.OFPAggregateStatsRequest(dp, 0,
ofp.OFPTT_ALL,ofp.OFPP_ANY,ofp.OFPG_ANY,cookie, cookie_mask,match)
